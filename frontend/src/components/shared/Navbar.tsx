...
export const Navbar = () => {
  ...
  const { currentMess, messes, setCurrentMess } = useMessStore();
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        messDropdownRef.current &&
        !messDropdownRef.current.contains(event.target as Node)
      ) {
  ...
  const handleMessSwitch = (mess: Mess) => {
    setCurrentMess(mess);
    setIsMessDropdownOpen(false);
    addToast(`Switched to ${mess.name}`, 'success', 3000);
    navigate('/dashboard');
  };
          ...
          <div className="flex items-center space-x-4">
            ...
            {currentMess && (
              <div className="relative" ref={messDropdownRef}>
                <button
                  onClick={() => setIsMessDropdownOpen(!isMessDropdownOpen)}
                  className="flex items-center space-x-2 px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors duration-200"
                  aria-label="Switch mess"
                  aria-expanded={isMessDropdownOpen}
                >
                    ...
                    <div className="py-2">
                      ...
                      <div className="border-t border-gray-200 mt-2 pt-2">
                        <button
                          ...
                          className="w-full text-left px-4 py-2 text-blue-600 hover:bg-gray-100 transition-colors duration-150 font-medium"
                        >
                          + Create or Join Mess
                        </button>
                      </div>
                ...
                <div className="absolute right-0 mt-2 w-56 bg-white rounded-lg shadow-lg border border-gray-200 z-50">
                  <div className="py-2">
                    ...
                    <button
                      ...
                      className="w-full text-left px-4 py-2 text-gray-800 hover:bg-gray-100 transition-colors duration-150 flex items-center space-x-2"
                    >
                      <svg
                        className="w-5 h-5"
                        fill="none"
